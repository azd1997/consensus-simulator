...
namespace ns3
{
  ...
  // 生成交易 每个交易1KB
  static void generateTX(int datasize, uint8_t data[])
  {
    //int size = num * tx_size;
    // // std::vector<char> data(size+1, '1');   // 定义num个交易内容拼接成的数组，初始化全为1
    // // data[size] = '\0';
    // uint8_t data[size];
    int i;
